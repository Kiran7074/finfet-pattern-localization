...
def apply_edge_brightening(
    ...
    """Brighten high-contrast feature edges."""
    ...
    # Add brightness around edges
    enhanced = img_f + strength * 255.0 * magnitude
...
def apply_rotation(
    ...
    """Rotate the image around its center while keeping the same dimensions."""
...
def image_reference(
    ...
    img = apply_edge_brightening(img,edge_brightening_strength)
...
def image_search(
    ...
    rotated = apply_rotation(downsampled,rotation_deg,)
    edge_brightened = apply_edge_brightening(rotated,edge_brightening_strength)
    drifted = apply_raster_drift(edge_brightened, shear_amplitude_px, drift_jitter_px, rng)
