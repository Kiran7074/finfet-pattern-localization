...
def parse_args():
    ...
    p.add_argument(
        "--detector-noise-search",
        type=float,
        default=GenerationParams.detector_noise_sigma_search
    )
    p.add_argument(
        "--edge-brightening-strength",
        type=float,
        default=GenerationParams.edge_brightening_strength
    )
...
def main():
    args = parse_args()
    start_time = time.perf_counter() # timer to count the syntthetc dataset generation time
    ...
    params = GenerationParams(
        ...
        detector_noise_sigma_search=args.detector_noise_search,
        edge_brightening_strength=args.edge_brightening_strength,
    )
    ...
    fieldnames = [
        ...
        "gt_box_x", "gt_box_y", "gt_box_w", "gt_box_h", "architecture","scale_ratio",
        "rotation_deg","edge_brightening_strength","beam_spot_size_nm", "collapse_threshold_nm", "dose_reference",
    ...
    print("=" * 60)
    print("DATASET GENERATION TIME")
    print("=" * 60)
    print(f"Samples       : {args.num_samples}")
    print(f"Time          : {elapsed:.2f} seconds")
    print(f"Time          : {elapsed / 60:.2f} minutes")
    print(f"Per sample    : {elapsed / args.num_samples:.4f} seconds")
    print("=" * 60)
